...
function Request(config) {
  // 3
  var uniqId = genReqUniqId(_extends({
...
// 根据请求配置生成自定义 headers
Request.dynamicHeaders = function () {};
...
var SAME_REQ_CANCELED = 'same_req_canceled';
...
function resInspector(res) {
  ...
  if (data.error) {
    // 7
    // 业务响应错误日志记录
    _log2.default.warn(_extends({
      name: '业务响应错误码',
      error: data.error
    }, errInfo));

    if (!config.preventHandleResError) {
      var bizErrType = buildBizErrTypeWithCode(data.error.code);
